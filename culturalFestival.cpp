...
int getKmaxSkillSum(vector<vector<int> >& skillMatrix, 
                    ...
                    const AT a){
    /// here find the best skill for each Art. Return the sum. and REMOVE the person from skill matrix
	
    //               elements of pq ,  underlying container   , comparator  
    priority_queue<pair<int,size_t> , vector<pair<int,size_t> >, CompareDist > topArtists; // maxheap
    set<size_t> tombStoned; //the indexes will stay sorted in here
    ...
    // now the top k people are the highest rated ones
    int k = artistsRequired;
    int maxSkill = 0;
    while(k--){
        auto top = topArtists.top();
		maxSkill += top.first;
		tombStoned.insert(top.second);
...
int main(){
    ...
    int i=0;
    while(i < totalPerson){ // make skill matrix
    ...
    for (auto onePattern: allPermutations){
        ...
        if(currentBestSum > maxTalentFound)    maxTalentFound = currentBestSum;
